...
const taskInProgress = useCallback((id) => {
    const inProgressTask = to_do.find(x => x.id === id);
    const task = {id: inProgress.length + 1, task: inProgressTask.task}
    setTo_do(prevState => prevState.filter((task) => task !== inProgressTask))
    setInProgress(prevState => [...prevState, task])
}, [inProgress.length, to_do])

const markAsDone2 = useCallback((id) => {
    const finishedTask = inProgress.find(x => x.id === id);
    const task = {id: done.length + 1, task: finishedTask.task}
    setInProgress(prevState => prevState.filter((task) => task !== finishedTask))
    setDone(prevState => [...prevState, task])
}, [done.length, inProgress])
        ...
        return (
            <div className="container">
                ...
                <div className="to-do">
                    <h1>To-Do :</h1>
{to_do.map((i) => (
<Todo key={i.id} id={i.id} task={i.task} action1={taskInProgress} action2={markAsDone} theme={theme}/>
))}
                </div>
                ...
                <div className="inProgress">
                    <h1>In Progress :</h1>
{inProgress.map((i) => (
<InProgress key={i.id} id={i.id} task={i.task} action={markAsDone2} theme={theme}/>
))}
                </div>
